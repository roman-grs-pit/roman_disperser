#!/usr/bin/env python
"""
Build simulated Roman dispersed images (grism or prism) from a unified
source catalog.
...
The dispersing element defaults to the G150 grism; select the P127 prism
with ``element: prism`` in the config (batch mode) or ``--element prism``
(quick mode). This script was named ``build_grism_image.py`` before prism
support; that name remains as a deprecated forwarding wrapper.
